...
const tiroDanno = roll(dannoFormula);
  ...
  if (incantesimo.attack_type === "ranged" || incantesimo.attack_type === "melee") {
...
🎯 Tiro per colpire: ${d20} + ${bonus} = ${totale} vs CA ${CA} → ${colpito ? "✅ Colpito" : "❌ Mancato"}
💥 Danno: ${danno || "?"}
...
📘 Effetto: ${incantesimo.desc?.[0] || "–"}`;
  ...
  return (
    <div className="widget" style={{ marginTop: "1rem" }}>
      ...
      <label>
  <input type="checkbox" checked={isMagico} onChange={(e) => setIsMagico(e.target.checked)} />
  {isMagico && entita.magia && (
  <div style={{ fontStyle: "italic", fontSize: "0.9em" }} title={`Stat magica: ${statMagica.toUpperCase()} (mod ${modStat})`}>
    🧠 Stat magica: <strong>{statMagica.toUpperCase()}</strong> → Bonus: <strong>{modStat}</strong>, CD incantesimo: <strong>{CD}</strong>
  </div>
)}
  ...
  <div style={{ marginTop: '1rem' }}>
    ...
    <ul>
      {Object.entries(entita.slots).map(([liv, qty]) => (
        <li key={liv}>Livello {liv}: {qty} slot</li>
      ))}
    </ul>
  </div>
)}
