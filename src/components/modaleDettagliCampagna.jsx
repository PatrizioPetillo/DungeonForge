    ...
    switch (tab) {
      case 'Generale':
        return (
          <div className="tab-content">
            <label>Titolo</label>
            <input value={dati.titolo} onChange={e => handleChange('titolo', e.target.value)} />
            <label>Tipo</label>
            ...
            <input value={dati.ambientazione} onChange={e => handleChange('ambientazione', e.target.value)} />
            <label>Stato</label>
      ...
      case 'Narrativa':
        return (
          <div className="tab-content">
            <h4>Prologo</h4>
            <textarea value={dati.prologo || ''} onChange={e => handleChange('prologo', e.target.value)} />
                        ...
                        <div key={j} className="blocco-scena">
                          ...
                          {scene.mostri?.length > 0 && (
                            <div className="blocco-mostri">
                              <p><strong>🐲 Mostri:</strong></p>
                              <ul>
                                {scene.mostri.map((mostro, k) => (
                                  <li key={k}>
                                    <strong>{mostro.nome}</strong> (GS {mostro.gs}, CA {mostro.ca}, PF {mostro.pf})<br />
                                    <em style={{ color: '#B2B9C3' }}>
                                      Fonte: {mostro.fonte === 'api' ? 'D&D 5e API' : 'Creato manualmente'}
                                    </em>
                                  </li>
                                ))}
                              </ul>
            ...
            <h4>Finale</h4>
            <textarea value={dati.finale || ''} onChange={e => handleChange('finale', e.target.value)} />
          </div>
        );
          ...
          <div className="tab-content">
            {dati.villain ? (
              <div className="villain-preview">
                <p><strong>{dati.villain.nome}</strong></p>
                <p>{dati.villain.titolo}</p>
                <p><em>{dati.villain.descrizione}</em></p>
                {modalitaModifica && (
                  <button onClick={() => console.log("Apertura Archivio Villain")}>🔁 Cambia Villain</button>
                )}
              </div>
            ) : (
      ...
      case 'PNG':
        return (
          <div className="tab-content">
            ...
            {modalitaModifica && (
              <button onClick={() => console.log("Apertura Archivio PNG")}>+ Modifica PNG</button>
            )}
          </div>
        );
              ...
              <ul className="lista-mostri">
                {dati.mostri.map((mostro, i) => (
                  <li key={i} className="mostro-item">
                    <h4>{mostro.nome}</h4>
                    <p>GS: {mostro.gs} | CA: {mostro.ca} | PF: {mostro.pf}</p>
                    <p>
                      <strong>Scena:</strong> {mostro.sceneName || 'Non assegnata'}
                      {mostro.sceneId && (
                        <button
                          className="vai-scena-btn"
                          onClick={() => {
                            const scenaTarget = document.getElementById(`scene-${mostro.sceneId}`);
                            if (scenaTarget) scenaTarget.scrollIntoView({ behavior: 'smooth' });
                          }}
                        >
                          📜 Vai alla Scena
                        </button>
                    ...
                    <p className="mostro-fonte">
                      Fonte: <em>{mostro.fonte === 'api' ? 'D&D 5e API' : 'Creato manualmente'}</em>
                    </p>
                  </li>
                ))}
              </ul>
    ...
    <div className="modale-overlay">
      <div className="modale-content">
        ...
        <div className="tab-selector">
          {['Generale', 'Narrativa', 'Villain', 'PNG', 'Luoghi', 'Incontri', 'Mostri'].map((t) => (
        ...
        <div className="modale-footer">
          {modalitaModifica ? (
            <>
              <button className="elimina-btn" onClick={eliminaCampagna}>🗑️ Elimina</button>
              <button className="salva-btn" onClick={salvaModifiche}>💾 Salva</button>
            </>
          ) : (
            <button className="modifica-btn" onClick={() => setModalitaModifica(true)}>✏️ Modifica</button>
          )}
        </div>
      </div>
    </div>
